...
const LIGHT_SURFACES = { bg950: '#f8fafc', bg900: '#ffffff' };

function applyTheme(colorName, mode) {
  ...
  if (mode === 'light') {
    root.style.setProperty('--surface-950', LIGHT_SURFACES.bg950);
    root.style.setProperty('--surface-900', LIGHT_SURFACES.bg900);
...
export function ThemeProvider({ children }) {
  ...
  useEffect(() => {
    if (activeOrg?.themeColor) {
      ...
      applyTheme(activeOrg.themeColor, mode);
    }
  }, [activeOrg]);
  ...
  async function updateThemeColor(colorName) {
    ...
    try {
      ...
      setThemeColor(prev);
      applyTheme(prev, mode);
      console.error('Failed to update theme:', err);
    }
  }

  return (
    <ThemeContext.Provider value={{ themeColor, updateThemeColor, COLOR_MAP, mode, toggleMode }}>
      {children}
